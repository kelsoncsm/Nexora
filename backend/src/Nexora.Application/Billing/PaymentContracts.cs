...
{
    Task<PlanPriceView> SetPriceAsync(Guid actor,CreatePlanPriceInput input,string correlationId,CancellationToken ct);Task<IReadOnlyList<PlanPriceView>> GetPricesAsync(CancellationToken ct);Task<CheckoutView> CreateCheckoutAsync(Guid actorUserId,Guid tenantId,CheckoutInput input,string correlationId,CancellationToken ct);Task<IReadOnlyList<InvoiceView>> GetTenantInvoicesAsync(Guid tenantId,CancellationToken ct);Task<IReadOnlyList<InvoiceView>> GetInvoicesAsync(CancellationToken ct);Task<IReadOnlyList<PaymentView>> GetPaymentsAsync(CancellationToken ct);Task<bool> ProcessWebhookAsync(string externalEventId,string eventType,string externalPaymentId,CancellationToken ct);
}
