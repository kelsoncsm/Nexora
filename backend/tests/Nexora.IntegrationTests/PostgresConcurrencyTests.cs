...
public sealed class PostgresConcurrencyTests
{
    ...
    public async Task AppointmentCreationIsAtomicPerTenantAndProfessional()
    {
        ...
        var key=Guid.NewGuid().ToString("N");var slug=$"pg-schedule-{key}";await using var factory=new PostgresApiFactory();await factory.InitializeAsync();using var client=factory.CreateClient();var token=await Register(client,$"{slug}@nexora.test");await CreateTenant(client,token,slug);client.DefaultRequestHeaders.Authorization=new AuthenticationHeaderValue("Bearer",await Select(client,token,slug));
