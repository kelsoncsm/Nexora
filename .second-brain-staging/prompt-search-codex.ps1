...
try {
    ...
    if ($eventData) {
        foreach ($field in @('prompt', 'user_prompt', 'message', 'text', 'input')) {
            if ($eventData.PSObject.Properties.Name -contains $field -and $eventData.$field) {
                $prompt = [string]$eventData.$field
                break
            }
        }
    }
    ...
    $stopwords = @(
        'ainda','algo','aquela','aquele','aqui','arquivo','assim','cada','coisa','com','como','da','das','de','depois','do','dos',
        'essa','esse','esta','este','fazer','isso','mais','mesmo','muito','na','nas','não','nos','para','pela','pelo','pode','poderia',
        'por','preciso','projeto','qual','quando','que','quero','ser','sobre','sua','tem','uma','usar','usando','você'
    )
    $terms = @($prompt.ToLowerInvariant() -split '[^a-z0-9áéíóúâêôãõç+#.-]+' | Where-Object {
    ...
    $matches = foreach ($category in $categories) {
        ...
        foreach ($file in Get-ChildItem -LiteralPath $directory -Filter '*.md' -File -ErrorAction SilentlyContinue) {
            ...
            $body = $content.ToLowerInvariant()
            $score = 0
            foreach ($term in $terms) {
                if ($name.Contains($term)) { $score += 5 }
                elseif ($body.Contains($term)) { $score += 1 }
            }
            if ($score -gt 0) {
                [PSCustomObject]@{ Score = $score; Category = $category; Name = $file.BaseName; Content = $content }
            }
        }
    }
    ...
    @{ additionalContext = $context } | ConvertTo-Json -Compress -Depth 6
} catch {
    Write-Output '{}'
}
